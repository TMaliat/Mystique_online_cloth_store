...
class Cart(object):
    ...
    def update(self, product_id, quantity=1):
        ...
        if str(product_id) not in self.session[self.cart_id]:
            self.session[self.cart_id][str(product_id)] = {"quantity": 0}

        updated_quantity = self.session[self.cart_id][str(product_id)].get('quantity', 0) + quantity
